...
# Output folder
OUTPUT_FOLDER_PATTERN = "GRU_LR{0}_BATCHSIZE{1}_NHIDDEN{2}/"
...
with tf.Session() as sess:
    for i in range(kk):
        ...
        np.save("gru_trainloss_"+str(i)+".npy",np.array(train_loss))
        np.save("gru_valloss_"+str(i)+".npy",np.array(validation_loss))
        np.save("gru_trainacc_"+str(i)+".npy",np.array(train_acc))
        np.save("gru_valacc_"+str(i)+".npy", np.array(validation_acc))
